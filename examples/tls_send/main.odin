...
main :: proc() {
	args := os.args[1:]
	if len(args) == 0 {
		fmt.eprintln("Usage: tls_send --host=<host> [options]")
		fmt.eprintln()
		fmt.eprintln("Options:")
		fmt.eprintln("  --host=<host>       SMTP server hostname (required)")
		fmt.eprintln("  --port=<port>       SMTP server port (default: 465)")
...
parse_args :: proc(args: []string) -> Config {
	cfg := Config {
		...
		body_text = "This is a test email sent with odin-smtp via TLS.",
		body_html = "<p>This is a test email sent with <b>odin-smtp</b> via TLS.</p>",
	}
	for arg in args {
		eq := -1
		...
		case "--port":      cfg.port = _parse_port(val, 465)
